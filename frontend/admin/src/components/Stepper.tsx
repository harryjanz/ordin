...
const S = {
  ...
  connector: { width: 1, height: 10, background: "rgba(var(--a-neutral-rgb),0.07)", marginLeft: 26 } as React.CSSProperties,
  navBase: {
    display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 10,
    border: "1px solid transparent", background: "transparent", width: "100%", textAlign: "left" as const,
    fontFamily: "'Inter', sans-serif", color: "rgba(var(--a-text-rgb),0.5)", cursor: "pointer",
  ...
  num: {
    ...
    background: "rgba(var(--a-neutral-rgb),0.07)", color: "rgba(var(--a-text-rgb),0.5)",
  } as React.CSSProperties,
  label: { fontSize: 13.5, fontWeight: 600 } as React.CSSProperties,
  sub: { fontSize: 11.5, color: "rgba(var(--a-text-rgb),0.5)", marginTop: 1 } as React.CSSProperties,
};
          ...
          <div key={step.label}>
            ...
            >
              <span
                style={{
                  ...S.num,
                  background: isCurrent ? "#9900ff" : isDone ? "rgba(93,212,144,0.16)" : "rgba(var(--a-neutral-rgb),0.07)",
                  color: isCurrent ? "#fff" : isDone ? "#5DD490" : "rgba(var(--a-text-rgb),0.5)",
                }}
              >
                {isDone ? "✓" : i + 1}
              </span>
              <span>
                <div style={{ ...S.label, color: isCurrent ? "var(--a-text)" : "rgba(var(--a-text-rgb),0.5)" }}>{step.label}</div>
