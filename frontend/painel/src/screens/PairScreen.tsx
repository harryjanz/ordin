...
interface Props {
  T: Theme;
  onDone: (company: CompanyInfo, token: string) => void;
}

// Adaptado de frontend/totem/src/screens/DevicePairingScreen.tsx — mesmo
// mecanismo de código+QR do pareamento de totem (ORD-042), reaproveitado
// pelo painel (ORD-119). Sem fallback de PIN — painel não tem login, só
// pareamento; se o código expirar, gera outro.
export default function PairScreen({ T, onDone }: Props) {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (r.data.status === "approved") {
          clearTimers();
          onDone(r.data.company, r.data.access_token);
      ...
      <div style={{
        ...
        {err ? (
          ...
          <>
            ...
            <div style={{ fontFamily: FONT_B, fontSize: FONT.body, color: T.muted }}>
              Expira em{" "}
