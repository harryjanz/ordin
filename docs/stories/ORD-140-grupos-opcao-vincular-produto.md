...
**3. Confirmação ao desvincular (02/09).** "Desvincular" era uma ação direta, sem aviso — pedido do usuário pra ter uma confirmação explicando a consequência. Adicionado `ConfirmDialog` (mesmo componente já usado em `CatalogScreen` pra exclusão de categoria/produto/cardápio) com mensagem nomeando o grupo e o produto, avisando que (a) as opções somem pra quem compra este produto, (b) o grupo continua na biblioteca e pode ser vinculado de novo, e (c) o máximo customizado pra este vínculo (ORD-144) é perdido se desvincular. Novo critério de aceite: "Desvincular exige confirmação explícita, com aviso do que acontece."
