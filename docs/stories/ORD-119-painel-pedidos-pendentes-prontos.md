...
- **Autorização nas rotas de leitura do order-service:** o novo token `role: "painel"` precisa ser aceito por `GET /orders?status=paid,ready` e pelo WebSocket `/ws/orders` — hoje esses endpoints esperam roles de staff (`get_current_user` com roles como `owner`/`manager`/`cashier`); adicionar `"painel"` à lista de roles aceitas nesses dois pontos específicos, sem acesso a nenhum outro endpoint de pedido (nada de criar/editar/coletar).
